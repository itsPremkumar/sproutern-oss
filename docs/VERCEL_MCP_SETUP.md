...
> Step-by-step guide to connecting Vercel MCP (Model Context Protocol) with OpenCode
> for AI-powered deployment management, log inspection, and Web Analytics.
>
> **Hermes note:** Hermes is not OpenCode. The OpenCode MCP steps below are for
> OpenCode's own use. Hermes manages Vercel **directly from the terminal via the
> Vercel CLI** (already installed, v55.0.0, logged in as `<your-vercel-username>`), which
> covers the same outcomes: deploy, read build/runtime logs, set env vars, query
> Web Analytics. Both paths work on this machine.
...
- OpenCode installed and configured (only for the MCP path)
...
```bash
vercel switch --scope <your-team-slug>
```
...
Replace `YOUR_TEAM_SLUG` with your team slug (e.g., `your-team-slug-abc123`).
