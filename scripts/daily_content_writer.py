...
def yaml_sq(s):
    """Safely wrap a string in YAML single quotes (escape inner quotes by doubling)."""
    return "'" + str(s).replace("'", "''") + "'"


def build_frontmatter(title, date, category, read_time, excerpt, keywords):
    kw = "\n".join(f"  - {yaml_sq(k.strip())}" for k in keywords if k.strip())
    return (
        "---\n"
        f"title: {yaml_sq(title)}\n"
        f"date: {yaml_sq(date)}\n"
        f"category: {yaml_sq(category)}\n"
        f"readTime: {yaml_sq(read_time)}\n"
        f"excerpt:\n  {yaml_sq(excerpt)}\n"
        f"author: {yaml_sq(AUTHOR)}\n"
        "keywords:\n"
        f"{kw}\n"
        "---\n\n"
    )
